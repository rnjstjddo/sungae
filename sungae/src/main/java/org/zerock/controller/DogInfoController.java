...
//	}
	...
	@PostMapping("/cart") // URI 입력시 분양리스트 화면만 보여준다.
	//public void dogcartpost(@RequestParam("dno") Long dno, Model model) {
	public String cartpost(DogBuyVO dogBuyVO, @RequestParam("dogdno") Long dogdno) {
	 	
		Long dno = dogdno;
		buyservice.dogregister3(dno);
		// service.get(dno);
...
//	이미 담긴상품입니다. 모달창을 사용하면 dogcart 페이지로 이미 이동된 상태다. -> 자바스크립트 사용하자. 
//	@PreAuthorize("isAuthenticated()")
//	@PostMapping("/cart") // URI 입력시 분양리스트 화면만 보여준다.
//	//public void dogcartpost(@RequestParam("dno") Long dno, Model model) {
//	public String cartpost(DogBuyVO dogBuyVO, RedirectAttributes rttr) {
//	 	buyservice.dogregister(dogBuyVO);
//		// service.get(dno);
//		
//		log.info("doglist 상품목록페이지에서 장바구니 등록 ");	
//		
//		Long listdno = dogBuyVO.getDno();
//			
//		//Long cartdno =dogBuyVO.getDno();
//		Long cartdno = buyservice.cartdno();
//		
//		if(cartdno == listdno) {
//			rttr.addFlashAttribute("cartdno", "이미 등록된 상품입니다.");
//			
//		}
//		
//		
